...
export async function GET() {
  try {
    ...
    // Convertir el stream a chunks con tipos correctos
    ...
    // Título principal
    doc.font('Helvetica-Bold').fontSize(30).fillColor('#2563eb').text('React 19 Cheatsheet', {
      align: 'center',
    })

    doc.moveDown(2)

    // Función para agregar un item del cheatsheet
    function addCheatSheetItem(item: CheatSheetItem, index: number) {
      ...
      // Descripción
      doc.font('Helvetica').fontSize(12).fillColor('#374151').text(item.description, {
        align: 'justify',
        continued: false,
      })
      ...
      if (item.code) {
        ...
        const textHeight = doc.heightOfString(item.code, {
          width: codeWidth - 20,
        })

        doc
          .fillColor('#f3f4f6')
          .rect(50, initialY, codeWidth, textHeight + 20)
          .fill()

        // Texto del código
        doc.fillColor('#374151').text(item.code, 60, initialY + 10, {
          width: codeWidth - 20,
          align: 'left',
        })
      }
    ...
    // Agregar cada item del cheatsheet
    cheatSheet.forEach((item, index) => {
      addCheatSheetItem(item, index)
    })
    ...
    for (let i = 0; i < pages.count; i++) {
      ...
      doc
        ...
        .fontSize(10)
        .fillColor('#6b7280')
        .text(`Página ${i + 1} de ${pages.count}`, 0, doc.page.height - 50, {
          align: 'center',
        })
    }
    ...
    return new Response(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename=react19-cheatsheet.pdf',
      },
    })
  } catch (error) {
